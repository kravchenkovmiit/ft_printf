...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/21 15:44:21 by hvalenci          #+#    #+#             */
/*   Updated: 2020/02/21 17:14:49 by hvalenci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "printf.h"

static int	inicialise_foos(char **conv, int **foo_type)
{
	*conv = "cspdiouxXf%";
...
static int	inicialise_flags(char **flags, int *f_type)
{
	int	i;
...
static int	process_conv(va_list args, char *str, int *i, int j)
{
	int		f_sz;
	char	*conv;
	char	*flags;
...
static int	process_format(va_list args, char *str, int *sz, int i)
{
	int		j;
	char	*buffer;
...
int			ft_printf(const char *format, ...)
{
	...
	sz = 0;
	str = (char*)format;
	if ((i = process_format(args, str, &sz, -1)) == -1)
		return (0);
	sz += i;
	va_end(args);
	return (sz);
